...
file_exposure = open(os.getcwd()+'\\exposure01.csv','w',newline = '')
...
file_universe = open(os.getcwd()+'\\universe01.csv', 'w', newline = '')
...
file_eligible = open(os.getcwd()+'\\eligible01.csv', 'w', newline = '')
...
file_fieldwork = open(os.getcwd()+'\\fieldwork01.csv', 'w', newline = '')
...
file_summaries = open(os.getcwd()+'\\summary01.csv', 'w', newline = '')
...
file_summaries2 = open(os.getcwd()+'\\summary02.csv', 'w', newline = '')
...
# For this demo, a separate file with that distribution was read and keep in memory 
  for further changes below in the script
'''
...
### GLOBAL FILE BETA rr
in_file = os.getcwd()+'\\lower_rr.txt'
...
'''
# The purging function (selecting Panelist3 instances that would not 
  be more part of panel so they won't be invited any more
...
# In fact this is bit more complex than a baseline: an iteration process was proposed to find the values of the composition of response rates attributed to Panelist3 instances
  until reaching a composition which in aggregate reflect the expected response rate of the population;
...
# The underlying assumption is that we would be better off by creating the current universe (the panel) by 
  a repeated process of recruitments and purgings, such as it happened for the large majority of consumer panels
...
def eligible(univ):
'''
# An eligible is a section of the universe that can be subjected to sampling at a particular period of time
...
'''
# The sampling procedure takes business rules for selecting those who can participate in a survey program, 
  given an expected response rate

# For this project there are also additional rules to manipulate some simple exceptions when the expected number
  of participants is not reached with the previous rule
'''
	n = target
	if rr_type == 2: ir = 0
...
# The underlying assumption is that we don't know which instance will get which status but we have an idea
  of the probability distribution of statuses which is faced against the rr attribute of the Panelist3 instance based on 
